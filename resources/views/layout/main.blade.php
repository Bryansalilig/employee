...
function updateTime() {
    ...
    timestamps.forEach(timestampElement => {
        ...
        if (minutes < 60) {
            ...
            // Display "X hours ago" if more than an hour
          if (hours === 1) {
        ...
        <div class="flexbox flex-1">
          ...
          <ul class="nav navbar-toolbar">
            ...
            <li class="dropdown dropdown-notification">
              ...
              <a class="nav-link dropdown-toggle" data-toggle="dropdown"><i class="fa fa-bell-o rel"><span class="{{ (hasUnread($userId)) ? 'notify-signal' : '' }}"></span></i></a>
              <ul class="dropdown-menu dropdown-menu-right dropdown-menu-media">
                <li class="dropdown-menu-header">
                  <div>
                    <span class="count" style="display:none">{{ notificationCount($userId) }}</span>
                    <span><strong><span class="total">{{ (Auth::check()) ? notificationCount(Auth::user()->id) : 0 }}</span> Unread</strong> Notifications</span>
            ...
            <li class="dropdown dropdown-user">
              ...
              <span id="user-id" data-id="{{ Auth::user()->id }}"></span>{{ ucfirst(strtolower(Auth::user()->first_name)) }}
