    ...
    $('.color-skin-box input:radio').change(function() {
        ...
        if(val != 'default') {
            if(! $('#theme-style').length ) {
                $('head').append( "<link href='css/themes/"+val+".css' rel='stylesheet' id='theme-style' >" );
            } else $('#theme-style').attr('href', 'css/themes/'+val+'.css');
...
{
    $.ajax({
        url: '/get-formatted-date', // Change this to your server route
        method: 'GET',
        data: { date: date, string: string },
        success: function(response) {
            var formattedDate = response.formatted_date;
            var convertedDate = timeConverter(formattedDate);
            // Use convertedDate in your UI
        },
